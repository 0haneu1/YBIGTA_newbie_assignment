...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h1 = X.dot(W1) + b1      # shape: (N, nn_hdim1)
        z1 = relu(h1)            # ReLU 활성화

        h2 = z1.dot(W2) + b2     # shape: (N, nn_hdim2)
        z2 = relu(h2)            # ReLU 활성화

        h3 = z2.dot(W3) + b3     # shape: (N, nn_hdim3)
        z3 = relu(h3)            # ReLU 활성화

        h4 = z3.dot(W4) + b4     # shape: (N, nn_output_dim)
        y_hat = sigmoid(h4)      # 시그모이드 활성화
        y_hat = y_hat.reshape(X.shape[0],)  # (N, 1)을 (N,)으로
